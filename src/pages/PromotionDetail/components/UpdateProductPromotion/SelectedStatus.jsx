...
export default function SelectedStatus(props) {
  ...
  const options = [
    { id: -1, name: 'Hủy' },
    { id: 0, name: 'Chờ xử lý' },
    { id: 1, name: 'Đang áp dụng' },
    { id: 3, name: 'Hết hạn' }
  ]
  const [statusSelected, setStatusSelected] = useState(options[1])
  const handleChangeStatus = (status) => {
    axios.post(`/api/promotion/status`, { code: id, status })
    .then(res => {
      if (res.data.success) {
        refreshData()
                ...
                {options && options.map((person) => (
                  ...
                  >
                    {({ selected, active }) => (
                      ...
                      {person.id === 3 && <div className="m-0 btn btn-transport" onClick={() => handleChangeStatus(person.id)}>{ person.name }</div>}
                      {person.id === 1 && <div className="m-0 btn btn-received" onClick={() => handleChangeStatus(person.id)}>{ person.name }</div>}
